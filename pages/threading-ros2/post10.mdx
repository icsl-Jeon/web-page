...
In the previous [article](https://motion-boseong.vercel.app/post9), we discussed
...
### 1. Callbacks in a same `MutuallyExclusive` group are exact the same with `SingleThreadedExecutor`.
...
Then, some might expect that the callbacks in `MutuallyExclusive` group are
working exactly the same as `SingleThreadedExecutor` performs the callbacks. For
a delayed processing of the callbacks, this is not true.
...
### 2. Of course, callbacks in different Mutually Exclusive group are not thread-safe.
...
For this testing, let us assume that `TimerCallback()` is not used, having only
two subscription callbacks.
...
Publisher node outputed the below:
...
```shell filename="MultiThreadedExecutor with MutuallyExclusive result"
...
Is is very clear: **callback for `long_topic` was not handled at all.** This is
very different from the round-robin results of `SingleThreadedExecutor` (please
ignore timestamps):
...
Apparently, the first subscription result processes the most recent new
reception (event) after it finished the previous callback which takes about
**2s**. This skipping behavior was reported in the
...
some waiting entities which became stale due to a
[mutex lock](https://github.com/ros2/rclcpp/blob/foxy/rclcpp/src/rclcpp/executors/multi_threaded_executor.cpp#L67).
...
initially it will not use mutexes initially. Thus, what we have is:
...
`subscriber_node` will use mutex by
