...
#### In works in sparse region, but not in dense spots.

However, in obstacle-dense regions, many shots may fail. In such cases, the
search relies on obstacle heuristics to guide the exploration. When a free space
...
In conclusion, while Hybrid A\* is a powerful pathfinding algorithm, its true
...
It is perfectly okay when applying this algorithm to real-time application,
showing order of msecs computing time in reasonable complex environments. For
more information, recommend to read this result in
