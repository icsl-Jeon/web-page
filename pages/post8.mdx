...
keywords: thread, executioner, ros2
---
...
- Things more difficult: assume another topic is being published at every
  **1s**, and the corresponding callback takes **4s** to complete. What will
  happen?
...
`short_topic` has around 20 characters. So it will takes about **2s**, and
`long_topic` will take **4s**. In this example, queue size for each topic is
**10**.
